...
<body class="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 min-h-screen text-slate-200 p-6">
    ...
    <nav class="glass rounded-xl px-6 py-4 mb-6 flex justify-between items-center max-w-7xl w-full mx-auto">
        ...
        <div class="flex gap-4 items-center">
            ...
            <a href="{{ route('users.create') }}" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg">Novo Usuário</a>
        </div>
    </nav>
                ...
                <tbody class="divide-y divide-slate-700/50">
                    @foreach($users as $user)
                        <tr class="hover:bg-slate-800/30 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded text-xs font-semibold {{ $user->role === 'admin' ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-500/20 text-slate-400' }}">
                                    {{ ucfirst($user->role) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right flex justify-end gap-2">
                                <a href="{{ route('users.edit', $user) }}" class="text-indigo-400 hover:text-indigo-300">Editar</a>
                                @if($user->id !== auth()->id())
