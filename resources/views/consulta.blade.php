...
<body class="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 min-h-screen text-slate-200 flex flex-col p-6">
    ...
    <nav class="glass rounded-xl px-6 py-4 mb-6 flex justify-between items-center max-w-7xl w-full mx-auto">
        ...
        <div class="flex gap-4 items-center">
            ...
            <a href="{{ route('consulta.index') }}" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600/80 hover:bg-indigo-500/80 rounded-lg transition-colors border border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.2)]">Consultar</a>

            <a href="{{ route('indicadores.index') }}" class="px-4 py-2 text-sm font-medium text-slate-300 hover:text-white bg-slate-800/50 hover:bg-slate-700/50 rounded-lg transition-colors border border-slate-700/50">Indicadores</a>
    ...
    <div class="max-w-7xl w-full mx-auto flex flex-col gap-6 flex-1">
        ...
        <div class="glass rounded-2xl shadow-xl p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-xs px-3 py-1 rounded-full {{ $totalResultados > 10000 ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-slate-700/50 text-slate-400' }}">
                    {{ number_format($totalResultados, 0, ',', '.') }} resultado(s)
                </span>
            </div>
            
            <form action="{{ route('consulta.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                
                <div class="flex flex-col relative">
                    <label class="text-xs text-slate-400 mb-1">Nome do Funcionário</label>
                    <input type="text" id="input-nome" name="nome" value="{{ request('nome') }}" placeholder="Ex: João da Silva" class="input-glass rounded-lg px-3 py-2 text-sm" autocomplete="off">
                </div>
                
                <div class="flex flex-col relative">
                    <label class="text-xs text-slate-400 mb-1">Centro de Custo (CC)</label>
                    <input type="text" id="input-cc" name="cc" value="{{ request('cc') }}" placeholder="Ex: 1024" class="input-glass rounded-lg px-3 py-2 text-sm" autocomplete="off">
                </div>

                <div class="flex flex-col relative">
                    <label class="text-xs text-slate-400 mb-1">Tipo de Ocorrência</label>
                    <input type="text" id="input-tipo" name="tipo_ocorrencia" value="{{ request('tipo_ocorrencia') }}" placeholder="Ex: Faltas" class="input-glass rounded-lg px-3 py-2 text-sm" autocomplete="off">
                </div>

                <div class="flex gap-2">
                    <div class="flex flex-col w-1/2">
                        <label class="text-xs text-slate-400 mb-1">Data Início</label>
                        <input type="date" name="data_inicio" value="{{ request('data_inicio') }}" class="input-glass rounded-lg px-3 py-2 text-sm [color-scheme:dark]">
                    </div>
                    <div class="flex flex-col w-1/2">
                        <label class="text-xs text-slate-400 mb-1">Data Fim</label>
                        <input type="date" name="data_fim" value="{{ request('data_fim') }}" class="input-glass rounded-lg px-3 py-2 text-sm [color-scheme:dark]">
                    </div>
                </div>

                <div class="flex flex-col relative">
                    <label class="text-xs text-slate-400 mb-1">Filial</label>
                    <input type="text" id="input-filial" name="filial" value="{{ request('filial') }}" placeholder="Ex: 1" class="input-glass rounded-lg px-3 py-2 text-sm" autocomplete="off">
                </div>

                <div class="flex flex-col">
                    <label class="text-xs text-slate-400 mb-1">Duração (HH:MM)</label>
                    <input type="text" name="duracao_hhmm" value="{{ request('duracao_hhmm') }}" placeholder="Ex: 08:00" class="input-glass rounded-lg px-3 py-2 text-sm">
                </div>

                <div class="flex flex-col lg:col-span-2 flex-row items-end justify-end gap-3 mt-1">
                    <a href="{{ route('consulta.index') }}" class="px-4 py-2 text-sm text-slate-300 hover:text-white transition-colors">Limpar Filtros</a>
                    
                    @if($totalResultados <= 10000)
                        <a href="{{ route('consulta.exportar', request()->query()) }}" id="btnExportar" onclick="this.classList.add('opacity-50','pointer-events-none');this.innerHTML='<svg class=\'animate-spin -ml-1 mr-2 h-4 w-4\' fill=\'none\' viewBox=\'0 0 24 24\'><circle class=\'opacity-25\' cx=\'12\' cy=\'12\' r=\'10\' stroke=\'currentColor\' stroke-width=\'4\'></circle><path class=\'opacity-75\' fill=\'currentColor\' d=\'M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z\'></path></svg>Exportando...'" class="px-4 py-2 text-sm font-medium text-indigo-400 border border-indigo-500/30 hover:bg-indigo-500/10 rounded-lg transition-colors flex items-center gap-2">
        ...
        <div class="glass rounded-2xl shadow-xl overflow-hidden flex-1 flex flex-col">
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        @forelse($ocorrencias as $ocorrencia)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                <td class="px-6 py-3 text-slate-400">#{{ $ocorrencia->id }}</td>
                                <td class="px-6 py-3">
                                    <div class="font-medium text-white">{{ $ocorrencia->funcionario->nome }}</div>
                                    <div class="text-xs text-slate-400">Filial: {{ $ocorrencia->funcionario->filial }}</div>
                                </td>
                                <td class="px-6 py-3 text-slate-300">{{ $ocorrencia->funcionario->cc }}</td>
                                <td class="px-6 py-3">
                                ...
                                <td class="px-6 py-3 text-slate-300">{{ $ocorrencia->dt_referencia }}</td>
                                <td class="px-6 py-3 text-slate-400">{{ $ocorrencia->inicio_origem ?: '-' }}</td>
                                <td class="px-6 py-3 text-slate-400">{{ $ocorrencia->fim_origem ?: '-' }}</td>
                                <td class="px-6 py-3 font-medium text-emerald-400">{{ $ocorrencia->duracao_hhmm ?: '-' }}</td>
                                <td class="px-6 py-3 font-medium text-amber-400">{{ $ocorrencia->total_geral ?: '-' }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="p-4 border-t border-slate-700/50 bg-slate-800/20 mt-auto">
                {{ $ocorrencias->appends(request()->query())->links('pagination::tailwind') }}
            </div>
        </div>
