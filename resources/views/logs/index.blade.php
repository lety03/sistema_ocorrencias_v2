...
<body class="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 min-h-screen text-slate-200 p-6">
    ...
    <nav class="glass rounded-xl px-6 py-4 mb-6 flex justify-between items-center max-w-7xl w-full mx-auto">
        ...
        <div class="flex gap-4 items-center">
            <a href="{{ route('consulta.index') }}" class="text-sm font-medium text-slate-300 hover:text-white">Voltar</a>
        </div>
    </nav>
                ...
                <tbody class="divide-y divide-slate-700/50">
                    @forelse($logs as $log)
                        <tr class="hover:bg-slate-800/30 transition-colors">
                            <td class="px-6 py-4 font-medium text-white">{{ $log->nome_arquivo }}</td>
                            <td class="px-6 py-4 text-slate-300">{{ $log->user_name }}</td>
                            <td class="px-6 py-4 text-emerald-400 font-bold">{{ $log->linhas_sucesso }}</td>
                            <td class="px-6 py-4 text-red-400 font-bold">{{ $log->linhas_erro }}</td>
                            <td class="px-6 py-4 text-slate-400 text-xs">{{ date('d/m/Y H:i:s', strtotime($log->created_at)) }}</td>
